...
% Call:
handle_call({get_distance, Ip}, From, State) ->
  case dict:find(Ip, State) of
    {ok, Val} ->
      {reply, Val, State};
    error ->
      spawn(
        fun() -> 
          ...
          gen_server:reply(From, Distance),
          gen_server:cast(?SERVER, {cache_measurement, Ip, Distance})
        end),
      {noreply, State}
  end;
...
% Casts:
handle_cast({cache_measurement, Ip, Val}, State) ->
  {noreply, dict:store(Ip, Val, State)};
...
calculate_distance(Ip) ->
  ...
  Hops = string:tokens(Trace, "\n"),
  length(Hops) - 1. % The first one is a header, hence minus 1
...
calculate_distance_test() ->
  ...
  erlymock:start(),
  erlymock:strict(pastry_utils, traceroute, [Ip], [{return, successful_trace()}]),
  erlymock:replay(), 
  ?assertEqual(18, calculate_distance(Ip)),
  erlymock:verify().
...
get_distance_test() ->
  ...
  erlymock:strict(pastry_utils, traceroute, [Ip], [{return, successful_trace()}]),
  erlymock:replay(), 
  ?assertEqual(18, distance(Ip)), % Performs the action
  ?assertEqual(18, distance(Ip)), % This time it should have been cached
  erlymock:verify(),
